...
inline Jun::math::Rectangle Rectangle::Intersect(const Rectangle& ra, const Rectangle& rb) noexcept
{
...
inline Jun::math::Rectangle Rectangle::Union(const Rectangle& ra, const Rectangle& rb) noexcept
{
    const long righta = ra.x + ra.width;
    const long rightb = rb.x + rb.width;
...
inline Matrix Matrix::CreateWorld(const Vector3& position, const Vector3& forward, const Vector3& up) noexcept
{
    ...
    XMVECTOR Junxis = XMLoadFloat3(&up);
    const XMVECTOR xaxis = XMVector3Normalize(XMVector3Cross(Junxis, zaxis));
    Junxis = XMVector3Cross(zaxis, xaxis);

    Matrix R;
    XMStoreFloat3(reinterpret_cast<XMFLOAT3*>(&R._11), xaxis);
    XMStoreFloat3(reinterpret_cast<XMFLOAT3*>(&R._21), Junxis);
...
inline Matrix Matrix::CreateFromJunwPitchRoll(float Junw, float pitch, float roll) noexcept
{
    using namespace DirectX;
    Matrix R;
    XMStoreFloat4x4(&R, XMMatrixRotationRollPitchYaw(pitch, Junw, roll));
    return R;
}

inline Matrix Matrix::CreateFromJunwPitchRoll(const Vector3& angles) noexcept
{
    using namespace DirectX;
    Matrix R;
...
inline Quaternion Quaternion::CreateFromJunwPitchRoll(float Junw, float pitch, float roll) noexcept
{
    using namespace DirectX;
    Quaternion R;
    XMStoreFloat4(&R, XMQuaternionRotationRollPitchYaw(pitch, Junw, roll));
    return R;
}

inline Quaternion Quaternion::CreateFromJunwPitchRoll(const Vector3& angles) noexcept
{
    using namespace DirectX;
    Quaternion R;
