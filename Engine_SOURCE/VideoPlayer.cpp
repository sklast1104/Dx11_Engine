...
namespace Jun {
	...
	public:
		...
		void update(float elapsed) {
			...
			if (pVideoSample) {
				...
				dbg("Sample count %d, Sample flags %d, sample duration %I64d, sample time %I64d. CT:%I64d. Buffer: %ld/%ldf\n", sampleCount, sampleFlags, llSampleDuration, llVideoTimeStamp, uct, buffCurrLen, buffMaxLen);
	...
	VideoPlayer::VideoPlayer()
		: Component(eComponentType::VideoPlayer)
	{
	}
	...
	bool VideoPlayer::CreateAPI()
	{
		HRESULT hr = MFStartup(MF_VERSION);
		return(hr == S_OK);
	}

	void VideoPlayer::DestoryAPI()
	{
		MFShutdown();
	}
	...
	{
		mData->update(Time::DeltaTime());
	}

	bool VideoPlayer::Create(const std::string& path)
	{
		assert(!mData);

		mData = std::make_shared<InternalData>();

		return mData->open(path.c_str());
	}
	...
	bool VideoPlayer::hasFinished()
	{
		assert(mData);

		return mData->finished;
	}
}