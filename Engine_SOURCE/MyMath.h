...
namespace Jun::math
{
    ...
    // Unary operators
        ...
        // Rotates about y-axis (Junw), then x-axis (pitch), then z-axis (roll)
        static Matrix CreateFromJunwPitchRoll(float Junw, float pitch, float roll) noexcept;

        // Rotates about y-axis (angles.y), then x-axis (angles.x), then z-axis (angles.z)
        static Matrix CreateFromJunwPitchRoll(const Vector3& angles) noexcept;
    ...
    struct Quaternion : public XMFLOAT4
    {
        ...
        // Rotates about y-axis (Junw), then x-axis (pitch), then z-axis (roll)
        static Quaternion CreateFromJunwPitchRoll(float Junw, float pitch, float roll) noexcept;

        // Rotates about y-axis (angles.y), then x-axis (angles.x), then z-axis (angles.z)
        static Quaternion CreateFromJunwPitchRoll(const Vector3& angles) noexcept;
    ...
    public:
        ...
        static RECT __cdecl ComputeDisplaJunrea(DXGI_SCALING scaling, UINT backBufferWidth, UINT backBufferHeight, int outputWidth, int outputHeight) noexcept;
#endif
